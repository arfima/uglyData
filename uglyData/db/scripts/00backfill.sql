...
CREATE OR REPLACE PROCEDURE decompress_backfill(staging_table regclass, 
    ...
    force_nulls boolean DEFAULT false,
    recompress_after boolean DEFAULT true)
AS $proc$
DECLARE
...
BEGIN
    ...
    ELSEIF UPPER(on_conflict_action) = 'UPDATE' THEN
        IF force_nulls THEN
            update_statement = '%1$I = EXCLUDED.%1$I';
        ELSE
            update_statement = '%1$I = COALESCE(EXCLUDED.%1$I,' ||dest||'.%1$I)';
        END IF;

        SELECT 'ON CONFLICT ' || on_conflict_target || ' DO UPDATE SET ' || STRING_AGG(FORMAT(update_statement, on_conflict_update_column), ', ')
    ...
    LOOP
        ...
        IF recompress_after AND chunks_decompressed THEN
